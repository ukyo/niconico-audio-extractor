...
export var SAMPLE_RATE_TABLE = [5512, 11025, 22050, 44100];
...
export var SAMPLE_SIZE_TABLE = [8, 16];

export var CHANNELS_TABLE = [1, 2];
...
var extractWave = (bytes: Uint8Array, tree: IFlv): IMedia => {
  ...
  var bodyLength =
    tags.map(tag => tag.bodyLength).reduce((a, b) => a + b) - tags.length;
